        ...
        <div class="" id="offcanvasAddUser" aria-labelledby="offcanvasAddUserLabel">
            <div class="card-header border-bottom">
                ...
                <div class="offcanvas-body mx-0 flex-grow-0">
                    <form class="add-new-user pt-0" id="addNewUserForm" action="{{route('admin.page.update', $data['rows']->hashid)}}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label" for="add-slide-title">Title</label>
                            <input type="text" class="form-control" id="add-slide-title" name="title" placeholder="Enter title" value="{{$data['rows']->title}}" value="{{old('title')}}" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label required" for="add-slide-description">Description</label>
                            <textarea class="form-control" id="add-slide-description" name="description" placeholder="Enter description" rows="3" value="{{old('description')}}">{{$data['rows']->description}}</textarea>
                        </div>
                        <div class="mb-3">
                            <label class="form-label required" for="add-news-content">Content</label>
                            <x-forms.tinymce-editor content="{{$data['rows']->content}}"/>
                        </div>
                        <div class="mb-4">
