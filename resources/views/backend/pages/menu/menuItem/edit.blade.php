        ...
        <div class="" id="offcanvasAddUser" aria-labelledby="offcanvasAddUserLabel">
            <div class="card-header border-bottom">
                <div class="offcanvas-header">
                    <h5 id="offcanvasAddUserLabel" class="offcanvas-title">Edit Menu</h5>
                </div>
                <div class="offcanvas-body mx-0 flex-grow-0">
                    <form class="add-new-user pt-0" id="addNewUserForm" action="{{route('admin.menu.item.update', $data['rose']->hashId)}}" method="POST">
                        @csrf
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="add-menuItem-parentId" name="parentId">
                                ...
                                @foreach ($data['menuItem'] as $menuItem)
                                    @if ($menuItem->haveChild)
                                        <option value="{{$menuItem->hashId}}" @if ($menuItem->id == $data['rose']->parent_id) selected @endif>{{$menuItem->label}}</option>
                                        @include('backend.pages.menu.menuItem.menuItemChild', ['menuitem' => $menuItem, 'level' => 1, 'selected' => $data['rose']->parent_id])
                                    @else
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control required" id="add-menuItem-href" name="href" placeholder="Enter href" value="{{ old('href', $data['rose']->href)}}" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label required" for="add-menuItem-order">Order</label>
                            <input type="number" class="form-control" id="add-menuItem-order" name="order" placeholder="Enter order" value="{{old('order', $data['rose']->order)}}">
                        </div>
                        <div class="mb-4">
                                ...
                                <div class="card-body">
                                    <div class="list-group list-group-flush">
                                        @foreach ($data['pages'] as $page)
                                            <a href="#" class="list-group-item list-group-item-action" data-href="">{{$page->title}}</a>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
