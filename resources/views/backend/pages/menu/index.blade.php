            ...
            <table class="datatables-users table border-top">
                <thead>
                    <tr>
                        ...
                        <th>Title</th>
                        <th>Status</th>
                        @if(auth()->user()->can('admin.menu.edit') || auth()->user()->can('admin.menu.delete') || auth()->user()->can('admin.menu.item.index'))
                            <th>Actions</th>
                        @endif
                    </tr>
                </thead>
                <tbody>
                    @foreach ($data['rose'] as $key => $item)
                        <tr>
                            ...
                            @if(auth()->user()->can('admin.menu.edit') || auth()->user()->can('admin.menu.delete') || auth()->user()->can('admin.menu.item.index'))
                            <td>
                                @can('admin.menu.item.index')
                                <a href="{{ route('admin.menu.item.index', $item->hasId) }}" class="btn btn-sm btn-primary mb-1 mt-1" title="View">
                                    <i class="fa fa-eye"></i>
                                </a>
                                @endcan
                                @can('admin.menu.edit')
                                <a href="{{ route('admin.menu.edit', $item->hasId) }}" class="btn btn-sm btn-primary mb-1 mt-1" title="Edit">
                                    <i class="fa fa-edit text-white"></i>
                                </a>
                                @endcan
                                @can('admin.menu.delete')
                                <form action="{{ route('admin.menu.delete', $item->hasId) }}" method="POST" class="d-inline-block">
                                    @csrf
