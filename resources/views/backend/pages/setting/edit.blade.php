        ...
        <div class="" id="offcanvasAddUser" aria-labelledby="offcanvasAddUserLabel">
            <div class="card-header border-bottom">
                ...
                <div class="offcanvas-body mx-0 flex-grow-0">
                    <form class="add-new-user pt-0" id="addNewUserForm" action="{{route('admin.setting.update', $data['rows']->hashId)}}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label" for="add-setting-title">Key: <b>{{$data['rows']->settings_key}}</b></label>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="add-setting-value">Value <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="add-setting-value" name="value" placeholder="Enter Value" value="{{ old('value') ?? $data['rows']->settings_value}}" required>
                        </div>
                        <div class="mb-4">
