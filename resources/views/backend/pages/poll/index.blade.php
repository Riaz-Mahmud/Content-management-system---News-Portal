            ...
            <table class="datatables-users table border-top">
                <thead>
                    <tr>
                        ...
                        <th>Options</th>
                        <th>Status</th>
                        @if(auth()->user()->can('admin.poll.edit') || auth()->user()->can('admin.poll.delete') || auth()->user()->can('admin.poll.item.index') || auth()->user()->can('admin.poll.result.index'))
                            <th>Actions</th>
                        @endif
                    </tr>
                </thead>
                <tbody>
                    @foreach ($data['rose'] as $key => $item)
                        <tr>
                            ...
                            <td>
                                @if (count($item->items) > 0)
                                    @foreach ($item->items as $option)
                                        <span class="badge bg-primary">{{ $option->option }}</span>
                                    @endforeach
                                @else
                                    N/A
                                @endif
                            ...
                            <td>
                                {{-- result --}}
                                @can('admin.poll.result.index')
                                <a href="{{ route('admin.poll.result', Crypt::encrypt($item->id)) }}" class="btn btn-sm btn-primary" title="Result">
                                    <i class="fas fa-eye mr-1"></i> Result
                                </a>
                                @endcan
                                @can('admin.poll.item.index')
                                <a href="{{ route('admin.poll.item.index', Crypt::encrypt($item->id)) }}" class="btn btn-sm btn-primary mb-1 mt-1" title="View">
                                    <i class="fa fa-eye"></i>
                                </a>
                                @endcan
                                @can('admin.poll.edit')
                                <a href="{{ route('admin.poll.edit', Crypt::encrypt($item->id)) }}" class="btn btn-sm btn-primary mb-1 mt-1" title="Edit">
                                    <i class="fa fa-edit text-white"></i>
                                </a>
                                @endcan
                                @can('admin.poll.delete')
                                <form action="{{ route('admin.poll.delete', Crypt::encrypt($item->id)) }}" method="POST" class="d-inline-block">
                                    @csrf
