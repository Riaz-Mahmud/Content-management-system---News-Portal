...
@section('title', 'Create Poll Item')
        ...
        <div class="" id="offcanvasAddUser" aria-labelledby="offcanvasAddUserLabel">
            <div class="card-header border-bottom">
                <div class="offcanvas-header">
                    <h5 id="offcanvasAddUserLabel" class="offcanvas-title">Add New Poll Item</h5>
                </div>
                <div class="offcanvas-body mx-0 flex-grow-0">
                    <form class="add-new-user pt-0" id="addNewUserForm" action="{{route('admin.poll.item.store', Crypt::encrypt($data['rose']->id))}}" method="POST" >
                        @csrf
                        <div class="mb-3">
                            <label class="form-label" for="add-new-poll-item">Option</label>
                            <input type="text" class="form-control" id="add-poll-option" name="option" placeholder="Enter option" value="{{old('option')}}" required maxlength="20">
                        </div>
                        {{-- position --}}
                        <div class="mb-3">
                            <label class="form-label required" for="add-new-poll-item">Position</label>
                            <input type="number" class="form-control" id="add-poll-position" name="position" placeholder="Enter position" value="{{old('position')}}" min="1" max="99" required>
                        </div>
                        <div class="mb-4">
