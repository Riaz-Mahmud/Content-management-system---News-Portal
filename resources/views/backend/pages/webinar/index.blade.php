...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="card-title">Webinar List</h5>
            ...
            <table class="datatables-users table border-top">
                <thead>
                    <tr>
                        ...
                        <th>Image</th>
                        <th>Status</th>
                        @if(auth()->user()->can('admin.webinar.edit') || auth()->user()->can('admin.webinar.delete'))
                            <th>Actions</th>
                        @endif
                    </tr>
                </thead>
                <tbody>
                    @foreach ($data['rows'] as $key => $item)
                        <tr>
                            ...
                            <td>
                                {{ Carbon\Carbon::parse($item->start_time)->format('d M Y g:i A') }}
                            </td>
                            <td>
                                {{ Carbon\Carbon::parse($item->end_time)->format('d M Y g:i A') }}
                            </td>
                            <td>
                            ...
                            @if(auth()->user()->can('admin.webinar.edit') || auth()->user()->can('admin.webinar.delete'))
                            <td>
                                @can('admin.webinar.edit')
                                <a href="{{ route('admin.webinar.edit', $item->hashId) }}" class="btn btn-sm btn-primary mb-1 mt-1" title="Edit">
                                    <i class="fa fa-edit text-white"></i>
                                </a>
                                @endcan
                                @can('admin.webinar.delete')
                                <form action="{{ route('admin.webinar.delete', $item->hashId) }}" method="POST" class="d-inline-block">
                                    @csrf
