        ...
        <div class="" id="offcanvasAddUser" aria-labelledby="offcanvasAddUserLabel">
            <div class="card-header border-bottom">
                ...
                <div class="offcanvas-body mx-0 flex-grow-0">
                    <form class="add-new-user pt-0" id="addNewUserForm" action="{{route('admin.webinar.update', $data['rows']->hashId)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label" for="add-webinar-title">Title <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="add-webinar-title" name="title" placeholder="Enter title" value="{{ old('title') ?? $data['rows']->title}}" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label required" for="add-webinar-description">Description</label>
                            <textarea class="form-control" id="add-webinar-description" name="description" placeholder="Enter description" rows="3">{{ old('description') ?? $data['rows']->description}}</textarea>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="add-webinar-link">Link <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="add-webinar-link" name="link" placeholder="Enter link" value="{{ old('link') ?? $data['rows']->link}}" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="add-webinar-password">Password Of Metting Link </label>
                            <input type="text" class="form-control" id="add-webinar-password" name="password" placeholder="Enter password" value="{{old('password') ?? $data['rows']->password}}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="add-webinar-start-date">Start Date <span class="text-danger">*</span></label>
                            <input type="datetime-local" class="form-control" id="add-webinar-start-date" name="start_date" placeholder="Enter start date" value="{{old('start_date') ?? $data['rows']->start_time }}" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="add-webinar-end-date">End Date <span class="text-danger">*</span></label>
                            <input type="datetime-local" class="form-control" id="add-webinar-end-date" name="end_date" placeholder="Enter end date" value="{{old('end_date') ?? $data['rows']->end_time}}" min="{{date('Y-m-d\TH:i')}}" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="add-webinar-image">Thumbnail</label>
