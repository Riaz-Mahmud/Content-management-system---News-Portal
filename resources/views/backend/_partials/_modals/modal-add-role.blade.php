    ...
    <div class="modal-dialog modal-lg modal-simple modal-dialog-centered modal-add-new-role">
        <div class="modal-content p-3 p-md-4">
            <div class="modal-body">
                ...
                <!-- Add role form -->
                <form id="addRoleForm" class="row g-3 addRoleForm" action="{{ route('admin.role.store') }}" method="POST">
                    @csrf
                                                ...
                                                <div class="d-flex">
                                                    @foreach ($permissions as $key1 => $permission)
                                                        <div class="form-check me-3 me-lg-5">
                                                            <input class="form-check-input" type="checkbox" name="permissions[]" value="{{$permission}}"  id="{{$permission}}{{$key1}}" />
                                                            <label class="form-check-label" for="{{$permission}}{{$key1}}">
                                                                {{$key1}}
                                                            </label>
                                                        </div>
                                                    @endforeach
